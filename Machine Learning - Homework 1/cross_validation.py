...
def split_cv(length, num_folds):
    ...
    tempvect = [num_folds*ii for ii in range(0,int(floor(len(indices)/num_folds)))]
    new_index = [[ indices[jj] for jj in range(ii, ii + num_folds)] for ii in tempvect]
    new_index = [list(x) for x in zip(*new_index)]

    for current_fold in range(num_folds):
        indexs = [ii for ii in range(0,num_folds)]
        indexs.pop(current_fold)
        test = new_index[current_fold]
        train = [new_index[ii] for ii in indexs]
        train = [j for i in train for j in i]
        splits[current_fold] = SplitIndices(train, test)
...
def cv_performance(x, y, num_folds, k):
    ...
    for split in splits:
        ...
        k_ = Knearest(x[split[0]], y[split[0]], k)
        c = k_.confusion_matrix(x[split[1]], y[split[1]])
        a = k_.accuracy(c)
